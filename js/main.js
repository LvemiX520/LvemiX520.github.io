  ...
  const scrollFn = function () {
    ...
    function percentageScrollFn(currentTop) {
      ...
      if (isInViewPortOfOneNoDis(pageBottomDomFlag || percentage > 90) && currentTop > 20) {
        ...
        // 对于导航和小工具挡住了以下代码添加了initial、middle、ends，还有source\css\_layout\nav.styl
        document.querySelector("#menus").className='ends'
      } else {
        $navTotop.classList.remove("long");
        $percentBtn.textContent = percentage;
        // 对于导航和小工具挡住了以下代码添加了initial、middle、ends，还有source\css\_layout\nav.styl
        document.querySelector("#menus").className='middle'
      }
    ...
    const scrollTask = anzhiyu.throttle(() => {
      ...
      if (currentTop > 26) {
        ...
        if (currentTop <= 5) {
          requestAnimationFrame(() => {
            // 对于导航和小工具挡住了以下代码添加了initial、middle、ends，还有source\css\_layout\nav.styl
            document.querySelector("#menus").className='initial';
